...
ubuntu, where the path is already right, so nothing had ever caught it. `lib.sh` gains `native()`
(`cygpath -m` where it exists, the identity elsewhere), which `aws-link.sh` had already been doing
inline for its one upload.
